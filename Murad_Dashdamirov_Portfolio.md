...
Drop these files in an `assets/` folder next to this document, then export the whole thing (resume + portfolio) to a single PDF under ~10 MB.

| File | Type | What to capture |
| --- | --- | --- |
| `assets/cpi-forecast.png` | PNG | Forecast page for one industry with the observed-vs-forecast chart and the driver breakdown both visible |
| `assets/cpi-architecture.png` | PNG | System diagram (render `docs/architecture.md` §1 via [mermaid.live](https://mermaid.live) or a screenshot; correct it to match the shipped code first) |
| `assets/cpi-scenario.gif` | GIF | Changing an eligible input and the baseline-vs-scenario forecast updating, then resetting (3–6s, compressed) |
| `assets/cpi-overview.png` | PNG | Overview page with the industry ranking table/chart |
| `assets/cpi-data-status.png` | PNG | Data Status page, or a screenshot of the ingestion quality report |

Capture on a clean browser window at consistent zoom with dev tools closed; keep diagrams grayscale to match the resume styling.
